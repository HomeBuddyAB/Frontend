...
export default function ForgotPasswordPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.success("If the email exists, a reset token has been generated.");
      if ((res.data as any)?.token) {
        setToken((res.data as any).token);
        toast.info("Dev mode: token returned (copy it).");
      } else {
        setToken(null);
      }
    } finally {
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#171010] px-4">
      <div className="w-full max-w-md rounded-2xl border border-[#362222] bg-[#1a1a1a] p-6 space-y-4">
        ...
        <p className="text-sm text-gray-300">
          Enter your email and we will generate a reset token. (Email delivery can be added later.)
        </p>
        <input
          value={email}
